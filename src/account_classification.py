...
class Account:
    def __init__(self, title: str, classification: str, statement: FinancialStatement, normal_balance):
        self.title = title
    ...
    def get_column_values(self):
        return [self.title, self.classification, self.statement.value, self.normal_balance.value]
...
def fuzzy_find(items: list[str], query: str):
    ...
    return min((item for item in items if normalize(item).startswith(query)), key = len, default=None)
...
def get_account_classification_table(sortby: list[str]):
    ...
    sortby = sortby_to_index(sortby)

    column_values = [account.get_column_values() for account in accounts]

    if sortby:
        column_values.sort(key = lambda row: [row[s] for s in sortby])
...
def main():
    ...
    if sortby == ["all"]:
        ...
        for column in column_names:
            print()
            out = get_account_classification_table(sortby)
    ...
    # Markdown header
    optional = "" if not sortby else f" (sorted by {', '.join(column_names[s] for s in sortby_to_index(sortby))})"
